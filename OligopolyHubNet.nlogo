...
to startup
  ...
  __hubnet-make-plot-narrowcast "Current Profit Plot in K"
  __hubnet-make-plot-narrowcast "Accumulated Profit Plot in M"
...
to setup
  ...
  set num-buyers-constant num-buyers
  set ideal-quantity-constant ideal-quantity
  ...
  ifelse count sellers = 0 [
   user-message "Can't run duopoly with no sellers. Please ask sellers to join."
   stop
  ][
  if count sellers = 1 [
        user-message "Can't run duopoly with 1 seller. You need more producers."
    stop
    ]
  ]
  ...
  if num-sellers != 0 [
    hubnet-broadcast "Current Profit in K" precision ((monopoly-profit / num-sellers) / 1000) 0
    hubnet-broadcast "Accumulated Profit in M" precision ((monopoly-profit / num-sellers) / 1000000) 0
    ]
...
;  hubnet-broadcast "extra-output" 0
  ...
  ;; Reset plots
  set-current-plot "Current Profit Plot in K"
  ask sellers [ __hubnet-clear-plot user-id ]

  set-current-plot "Accumulated Profit Plot in M"
   ask sellers [ __hubnet-clear-plot user-id ]
...
;  set age-of-news 0
  set avg-price 0
  set min-price 0
  set monopoly-profit 0;( monopoly-quantity * ( monopoly-price - marginal-cost) )
  set time-period 0
end
...
;             send-news-item word user-id " unleashes reserves as warning"
          ...
          ;; to produce it.
          [ if strategy = "Price >= MC" [set price-1 monopoly-price ;print "101"
            set p1-quantity-offered 0 ;print "102"
            if time-period = 0 [
              set p2-quantity-offered individual-quota; print "103"
              set price-2 monopoly-price ;print "104"
              set prev-quantity-offered p2-quantity-offered; print "105"
            ]
            ...
            [ ifelse (price-2 - 10) > marginal-cost
               [ set price-2 (price-2 - 10); print "106"
                 ]
               [ set price-2 marginal-cost ;print "107"
                 ]  ;; but don't go as far as pricing below cost
              set p2-quantity-offered prev-quantity-offered ;print "108"
            ]
            ;;if you did sell more that last time, increase production even a little more
            ;; (as long as price > mc)
            [ if price-2  > marginal-cost [
                set p2-quantity-offered (p2-quantity-offered + 10) ;print "109" ;; amt offered keeps increasing
               ]
            ]
...
to update-sellers
  ask sellers [
    ...
    set accumulated-profit accumulated-profit + last-sale-profit
   ]
end
...
to execute-command [command]
    ...
    [ ask sellers with [user-id = hubnet-message-source]
      ...
      [ ifelse command = "extra-output" [ask sellers with [user-id = hubnet-message-source]
          [
...
to execute-transactions
   ...
   ;; before executing transactions, ask sellers to record how much they sold last time
    ...
    ask sellers [
      set prev-p2-quantity-sold p2-quantity-sold ;print "201"
      set p1-quantity-sold 0; print "202"
      set p2-quantity-sold 0 ;print"203"
    ]

    ask buyers [
    set quantity-bought 0 ;print ;"204";; initialize quantity-bought this time period

    ifelse min-price <= max-price
      [ set quantity-demanded ideal-quantity-constant ;print"205"
        ]
      [ set quantity-demanded 0 ;print"206"
        ]

    ;; try to buy amount you demand from sellers
    buy-from-sellers ; print"207"
    ...
    [ setxy ([one-of (list (xcor + random-float 1.5) (xcor - random-float 1.5))]
     ...
     ; print"208"
      ...
      [ setxy random-pxcor random-pycor; print"209"
        ]
   ]
end
...
;10
     ...
     ;; but need to check to see if official price or the side price is lower first
      ifelse [price-1] of lucky-one = min-price
        [ set avail-from-seller [p1-quantity-available] of lucky-one ;print "1"
          ]
        [ ifelse [price-2] of lucky-one = min-price
          [ set avail-from-seller [p2-quantity-available] of lucky-one ;print "2"
            ]
          [ set avail-from-seller 0 ;print "3"
            ]
        ]
     ...
     ifelse avail-from-seller >= quantity-demanded
       [ set quantity-bought (quantity-bought + quantity-demanded) ;print "4"
         ]
       [ set quantity-bought (quantity-bought + avail-from-seller) ;print "5"
         ]

     ;; update info of seller you just bought from
     set amt-just-sold quantity-bought ;print "6"
     ask lucky-one [
      ifelse [price-1] of lucky-one = min-price
         [ set p1-quantity-available (p1-quantity-available - amt-just-sold)  ;; decrement seller's remaining amt available at p1
           set p1-quantity-sold (p1-quantity-sold + amt-just-sold)  ;print "7" ;; increment seller's amt purchased by buyers at p1
         ]
         [ if [price-2] of lucky-one = min-price
           [ set p2-quantity-available (p2-quantity-available - amt-just-sold)  ;; decrement seller's remaining amt available at p2
             set p2-quantity-sold (p2-quantity-sold + amt-just-sold) ;print "8"  ;; increment seller's purchased by buyers at p2
           ]
         ]
     ]

   ;; update your own info
   set quantity-demanded (quantity-demanded - quantity-bought) ;print "9"
   set seller-bought-from lucky-one ;print "10"
   ...
   [ ifelse any? sellers with [p1-quantity-available > 0 or p2-quantity-available > 0]
         [ find-lowest-offer                   ;; of the sellers with capacity, find the lowest priced ones
           buy-from-sellers       ;print "11"             ;; try to buy from them
         ]
         [stop ;print "12"
           ]  ;; you've tried all the sellers
   ]
...
;  hubnet-broadcast "# Suppliers" num-sellers
  ...
  ask sellers [
    hubnet-send user-id "Current Profit in K" precision (last-sale-profit / 1000) 0
;    hubnet-send user-id "Marginal Cost" marginal-cost
    hubnet-send user-id "Profit Needed in K" precision (profit-needed / 1000) 0

    ifelse time-period = 0 [
    hubnet-send user-id "Current-Need Difference in K" precision ((monopoly-profit / num-sellers - profit-needed ) / 1000) 0
    ][
    hubnet-send user-id "Current-Need Difference in K" precision (last-sale-profit / 1000 - profit-needed / 1000) 0
    ]

    hubnet-send user-id "Accumulated Profit in M" precision (accumulated-profit / 1000000) 0
...
to plot-current-profit
  set-current-plot "Current Profit Plot in K"
  ask sellers [
    __hubnet-plot user-id last-sale-profit / 1000
  ]
end

to plot-accumulated-profit
  set-current-plot "Accumulated Profit Plot in M"
  ask sellers [
    __hubnet-plot user-id accumulated-profit / 1000000
  ]
end
...
to plot-current-p1-quantity
  set-current-plot "Quantity Sold at Official Price"
  plot sum [p1-quantity-sold] of sellers with [p1-quantity-sold > 0]
;  ask sellers [
;    set-current-plot-pen word "Seller " who
...
to plot-current-p2-quantity
  set-current-plot "Quantity Sold at Lower Price"
    plot sum [p2-quantity-sold] of sellers with [p2-quantity-sold > 0]
;  ask sellers [
;    set-current-plot-pen word "Seller " who
...
Strategy
Strategy
"Agreement" "Price >= MC" "Quota-Plus" "Flood Market" "Quota-Minus"
0
...
316
177
Reduced output amount \nwhen using Quota-Minus
11
0.0
1
